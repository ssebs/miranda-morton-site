...
**Contact form** (`ContactForm.astro`) posts to **FormSubmit.co** — no backend. It mirrors the field/hidden-input structure of ssebs.com/contact (`name`, `email`, `_subject`, `message`, hidden `_captcha` and `_next`). The owner's email and LinkedIn live in one place, `src/lib/site.ts`, and are imported wherever they're shown (footer, contact page, contact form's FormSubmit target) — edit them there. The success banner is revealed client-side from a `?sent=1` query param (the static page can't read it at build time).
